    ...
    */
    function saveConfig()
    {
        ...
        if (db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE `w_configuracion` SET `titulo` = \'' . $c['titulo'] . '\', `slogan` = \'' .
            ...
            '\', `offline_message` = \'' . $c['offline_message'] . '\', `c_recaptcha` = \'' . $c['recaptcha'] . '\', `pkey` = \'' . $c['pkey'] . '\', `skey` = \'' . $c['skey'] . '\', `pkey3` = \'' . $c['pkey3'] . '\', `skey3` = \'' . $c['skey3'] . '\' WHERE `tscript_id` = \'1\''))
            return true;
        else
    ...
    */
    ...
    function newEmoticon() {
        ...
        $e = array(
            ...
            'smile' => $tsCore->setSecure($_POST['smile'])
        );
        ...
        return db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `w_emoticones` (`code`, `smile`) VALUES (\''.$e['code'].'\', \''.$e['smile'].'\')');
    }
    function saveEmoticon() {
        ...
        return db_exec(array(__FILE__, __LINE__), 'query', 'UPDATE w_emoticones SET code = \''.$e['code'].'\', smile = \''.$e['smile'].'\' WHERE id = \''.(int)$e['id'].'\'');
    }
