								...
								{if $tsSave}<div style="display: block;" class="mensajes ok">Tus cambios han sido guardados.</div>{/if}																{if $tsDelete == 'true'}<div style="display: block;" class="mensajes ok">Noticia eliminada.</div>{/if}
									...
									<fieldset>
										...
										<dl>
											<dt><label class="form-label" for="ai_new">Noticia:</label><br /><span>Puedes utilizar los siguentes BBCodes [url], [i] [b] y [u]. El m&aacute;ximo de caracteres permitidos es de <b>190</b>.</span></dt>
											<dd><textarea name="not_body" id="ai_new" rows="3" cols="50">{$tsNew.not_body}</textarea></dd>
										</dl>
										<dl>
											<dt><label class="form-label" for="ai_not_active">Activar noticia:</label><br /><span>Activar inmediatamente esta noticia en {$tsConfig.titulo}.</span></dt>
											<dd>
