...
var muro = {
    ...
    more_comments: function(id, obj){
        // LOADER / STATUS
        muro.stream.status = 1;
        $(obj).parent().find('loading-lg').show();
        //
        $('#loading').fadeIn(250); 
        $.ajax({
        	type: 'POST',
...
$(function(){
    ...
    // ADJUNTAR
    $('.adj').click(function(){ var aid = $(this).attr('aid'); })
    //
...
function ModalFoto(url) {
   $('#loading').fadeIn(250);
   $('.modal.active').addClass('modal-lg');
   $.ajax({
      type: 'POST',
      data: 'url=' + url,
      success: function() {
...
function ModalVideo(url) {
    ...
    $.ajax({
      ...
      success: function() {
        ...
        mydialog.body('<iframe width="100%" height="350" src="https://www.youtube.com/embed/' + url + '" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>');
        mydialog.buttons(true, true, 'Aceptar', 'close', true, true);
        mydialog.center();
        $('#loading').fadeOut(350);
      }
