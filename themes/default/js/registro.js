...
var registro = {
    ...
    focus: function(el) {
        ...
        this.show_status(el, 'text-info', $(el).attr('title'), true);
    },
        ...
        switch (campo) {
            ...
            case 'nick':
                ...
                if (value.length < 4 || value.length > 16)
                    return this.show_status(el, 'text-error', 'Debe tener entre 4 y 16 caracteres');

                //No solo numeros
                if (!/[^0-9]/.test(value))
                    return this.show_status(el, 'text-error', 'No puede contener solo numeros');

                //Caracteres validos
                if (/[^a-zA-Z0-9_]/.test(value))
                    return this.show_status(el, 'text-error', 'S&oacute;lo se permiten letras, n&oacute;meros y guiones(_)');

                //Contrase̱a === nick
                //              if(value === this.datos['password'])
                //                  return this.show_status($, 'text-error', 'La contrase&ntilde;a tiene que ser distinta que el nick');
                ...
                if (!this.cache[campo]) {
                    ...
                    if (this.cache[campo][value_lower]['status'])
                        return registro.show_status(el, 'text-success', this.cache[campo][value_lower]['text']);
                    else
                        return registro.show_status(el, 'text-error', this.cache[campo][value_lower]['text']);
                }
                    ...
                    success: function(h) {
                        ...
                        switch (h.charAt(0)) {
                            case '0': //Estaba en uso
                                ...
                                registro.show_status(el, 'text-error', h.substring(3));
                                break;
                            case '1': //No esta en uso
                                registro.cache[campo][value_lower]['status'] = true;
                                registro.show_status(el, 'text-success', h.substring(3));
                                break;
                        }
                        $('#loading').fadeOut(350);
                    },
                    error: function() {
                        registro.show_status(el, 'text-error', 'Hubo un error al intentar procesar lo solicitado');
            ...
            case 'password':
                ...
                if ($.inArray(value.toLowerCase(), this.banned_passwords) != -1)
                    return this.show_status(el, 'text-error', 'Ingresa una contrase&ntilde;a m&aacute;s segura');

                //Contrase̱a === nick
                if (value === this.datos['nick'])
                    return this.show_status(el, 'text-error', 'La contrase&ntilde;a tiene que ser distinta que el nick');

                //Tama̱o
                if (value.length < 5 || value.length > 35)
                    return this.show_status(el, 'text-error', 'Debe tener entre 5 y 35 caracteres');

                //OK
                return this.show_status(el, 'text-success', 'OK');
                break;
            ...
            case 'password2':
                ...
                if (value !== this.datos['password']) {
                    this.show_status($('#RegistroForm #password'), 'text-error', 'Las contrase&ntilde;as deben ser iguales');
                    return this.show_status(el, 'text-error', 'Las contrase&ntilde;as deben ser iguales');
                }
            ...
            case 'email':
                ...
                //Tama̱o
                if (value.length > 35)
                    return this.show_status(el, 'text-error', 'El email es demasiado largo');

                //Caracteres validos
                if (!/^[_a-z0-9-]+(.[_a-z0-9-]+)*@[a-z0-9-]+(.[a-z0-9-]+)*(.[a-z]{2,3})$/.exec(value))
                    return this.show_status(el, 'text-error', 'El formato es incorrecto');
                ...
                if (!this.cache[campo]) {
                    ...
                    if (this.cache[campo][value]['status'])
                        return registro.show_status(el, 'text-success', this.cache[campo][value]['text']);
                    else
                        return registro.show_status(el, 'text-error', this.cache[campo][value]['text']);
                }
                    ...
                    success: function(h) {
                        ...
                        switch (h.charAt(0)) {
                            ...
                            case '1': //No esta en uso
                                ...
                                registro.show_status(el, 'text-success', 'OK');
                                break;
                        }
                        $('#loading').fadeOut(350);
                    },
            ...
            case 'anio':
                ...
                if (!empty(this.datos['dia']) && !empty(this.datos['mes']) && !empty(this.datos['anio'])) {
                    ...
                    if (!checkdate(this.datos['mes'], this.datos['dia'], this.datos['anio']))
                        return this.show_status(el, 'text-error', 'La fecha es incorrecta');
                    //Compruebo que tenga mas de 18 a̱os
                    /*if(edad(this.datos['mes'], this.datos['dia'], this.datos['anio']) < 18)
                        return this.show_status(el, 'text-error', 'Tienes que tener por lo menos 18 a&ntilde;os para registrarte');*/
            ...
            case 'pais':
                ...
                this.show_status(el, 'text-success', 'OK');
            ...
            case 'terminos':
                ...
                return registro.show_status(el, 'text-success', 'OK');
                break;
            ...
            case 'g-recaptcha-response':
                ...
                return registro.show_status($('#RegistroForm .g-recaptcha'), 'text-success', 'OK');
                break;
        }
    },
    ...
    show_status: function(el, status_aux, text, no_cache_data) {
        var campo = $(el).attr('name');
        var status = (status_aux == 'empty') ? 'text-error' : status_aux;
        ...
        $(el).removeClass('text-success').removeClass('text-error').removeClass('text-info').removeClass('loading').addClass(status).show().children().children().html(text);
            ...
            success: function(h) {
               ...
               switch (h.substring(0, strpos(h, ':'))) {
                    ...
                    case 'nick': //Error nick
                        ...
                        registro.show_status($('#RegistroForm #nick'), 'text-error', h.substring(strpos(h, ':') + 2));
                        break;
                    case 'password': //Password
                        registro.change_paso(1, true);
                        registro.show_status($('#RegistroForm #password'), 'text-error', h.substring(strpos(h, ':') + 2));
                        registro.datos['password'] = '';
                        break;
                        //
                    case 'email': //Email
                        registro.change_paso(1, true);
                        registro.show_status($('#RegistroForm #email'), 'text-error', h.substring(strpos(h, ':') + 2));
                        break;
                    case 'nacimiento': //Dia|Mes|Anio
                        registro.change_paso(1, true);
                        registro.show_status($('#RegistroForm #anio'), 'text-error', h.substring(strpos(h, ':') + 2));
                        break;
                    case 'sexo': //Sexo
                        registro.change_paso(2, true);
                        registro.show_status($('#RegistroForm #sexo_f'), 'text-error', h.substring(strpos(h, ':') + 2));
                        break;
                    case 'pais': //Pais
                        registro.change_paso(2, true);
                        registro.show_status($('#RegistroForm #pais'), 'text-error', h.substring(strpos(h, ':') + 2));
                        break;
                    case 'estado': //Estado
                        registro.change_paso(2, true);
                        registro.show_status($('#RegistroForm #estado'), 'text-error', h.substring(strpos(h, ':') + 2));
                        break;
                        /*case 'ciudad': //Ciudad
                            registro.change_paso(2, true);
                            registro.show_status($('#RegistroForm #ciudad'), 'text-error', h.substring(strpos(h, ':')+2));
                            break;*/
                    case 'recaptcha': //reCAPTCHA
                        registro.change_paso(2, true);
                        registro.show_status($('#RegistroForm .g-recaptcha'), 'text-error', h.substring(strpos(h, ':') + 2));
                        break;
                    case '1':
                        if (registro.dialog) {
